...
# endif
  ...
  checkpoint_buf= static_cast<byte*>(aligned_malloc(get_block_size(),
                                                    get_block_size()));
  if (!checkpoint_buf)
  {
  ...
  mtr_t::finisher_update();
  memset_aligned<64>(checkpoint_buf, 0, get_block_size());
                     ...
                     const completion_callback *callback)
{
  ...
  ut_ad(lsn != LSN_MAX);
  ut_ad(lsn != 0);
  ut_ad(!log_sys.is_mmap() || !callback);
...
ATTRIBUTE_COLD void log_t::set_mmap(bool mmap)
{
  ...
  {
    alignas(16) byte log_block[max_block_size];
    const size_t bs{get_block_size()};
