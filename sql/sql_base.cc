...
{
  if (table->triggers->process_triggers(table->in_use, TRG_EVENT_UPDATE, TRG_ACTION_BEFORE, TRUE) ||
                                     ...
                                     enum trg_event_type event)
{
  ...
  if (!result && table->triggers)
    result= invoke_before_triggers(thd, table, &fields, &values, ignore_errors, event);
  return result;
}
