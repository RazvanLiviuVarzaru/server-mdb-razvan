...
int handler::bulk_update_row_optimized(const uchar * old_data, const uchar * new_data)
{
...
{
  return table->triggers->process_triggers(table->in_use, TRG_EVENT_UPDATE, TRG_ACTION_AFTER, TRUE);
}
...
{
  return ::cut_fields_for_portion_of_time(table->in_use, table, table->pos_in_table_list->period_conditions);
}


int handler::invoke_before_triggers(const uchar *, const uchar *)
{
  return ::invoke_before_triggers(table->in_use, table, fields, values, false, TRG_EVENT_UPDATE);
}
...
handler::Update_execution_markup build_update_execution_plan(handler::Update_func funcs[MAX_UPDATE_FUNCS],
                                                             TABLE_LIST *tl,
                                                             TABLE_SHARE *table_share,
                                                             handler *h,
                                                             bool with_bulk, bool has_vers_fields, bool using_limit)
{
  ...
  if ((triggers && triggers->has_triggers(TRG_EVENT_UPDATE, TRG_ACTION_BEFORE)) || tl->table->vfield)
  ...
  */
  ...
  if (table_share->versioned == VERS_TIMESTAMP && tl->table->in_use->lex->sql_command == SQLCOM_DELETE)
...
update_begin:
  ...
  */
  ...
  update_execution_markup= build_update_execution_plan(update_execution_plan, table_list, table->s, table->file,
  ...
  while (!(error=info.read_record()) && !thd->killed)
  {
    ...
    if (!select || select->skip_record(thd) > 0)
    {
      ...
      do
      {
        error= (handler->*update_execution_plan[f])(table->record[1], table->record[0]);
        if (unlikely(error))
        {
      ...
      if (unlikely(error) &&
          (!ignore || table->file->is_fatal_error(error, HA_CHECK_ALL) || error < HA_ERR_FIRST))
      {
        /*
          If (ignore && error is ignorable) we don't have to do anything; otherwise...
        */
